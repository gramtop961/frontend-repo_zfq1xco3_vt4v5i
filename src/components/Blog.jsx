...
export default function Blog() {
  return (
    <section id="blog" className="relative bg-black py-20 text-white">
      <div className="mx-auto max-w-7xl px-6">
        <div className="flex items-end justify-between">
          ...
          <a href="#" className="hidden items-center gap-2 text-sm text-yellow-300 hover:text-yellow-200 sm:inline-flex">
            View all
        ...
        <div className="mt-10 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {posts.map((p) => (
            <article key={p.title} className="group overflow-hidden rounded-2xl border border-white/10 bg-white/5 transition hover:border-yellow-400/50 hover:bg-white/10">
              <div className="aspect-[16/9] bg-gradient-to-tr from-yellow-400/20 via-yellow-300/10 to-white/10" />
              <div className="p-6">
                <div className="mb-2 flex items-center gap-2 text-xs text-white/60">
                  <span className="rounded-full bg-white/10 px-2 py-0.5 text-white/70">{p.tag}</span>
                  <span>{p.date}</span>
                </div>
                <h3 className="text-lg font-semibold group-hover:text-yellow-300">{p.title}</h3>
                <p className="mt-2 text-sm text-white/70">{p.excerpt}</p>
                <a href="#" className="mt-4 inline-flex items-center gap-2 text-sm text-yellow-300 hover:text-yellow-200">
                  Read more
                  <ArrowRight className="h-4 w-4" />
                </a>
              </div>
